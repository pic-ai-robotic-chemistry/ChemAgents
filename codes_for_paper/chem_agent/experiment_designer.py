...
from chem_prompt.prompt import ED_SYSTEM_PROMPT,ED_SELFREFLECT_PROMPT, ED_EXPERTRULE_PROMPT,EXAMPLE_PROMPT
...
def ED(message):
    ...
    agent.reset_prompt(node_name="aa",refine_prompt=ED_SELFREFLECT_PROMPT)
    ...
    agent.reset_prompt(node_name="bb",refine_prompt=ED_EXPERTRULE_PROMPT)
    for event in agent.graph.stream({"messages": [("user", value["messages"][-1].content)]},config=agent.config):
        for value in event.values():
            if isinstance(value["messages"][-1], BaseMessage):
                # if value["messages"][-1].type == "ai" and value["messages"][-1].content != "":
